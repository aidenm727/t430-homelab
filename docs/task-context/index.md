...
Returned blob content remains exact raw bytes. B1a does not decode, parse, select, transform, summarize, hash as source content or payload, apply a budget, or construct a package. Checkpoint B1b remains unauthorized for parsing, relationship verification, and selection reasoning. Checkpoint B2 remains unauthorized for compilation, integrity validation, explanation, budgeting, and golden replay. Protected-branch content remains out of scope.
